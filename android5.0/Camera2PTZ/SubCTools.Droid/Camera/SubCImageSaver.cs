...
namespace SubCTools.Droid.Camera
{
    ...
    public class SubCImageSaver : IImageSaver
    {
        public void SaveJpeg(Image image, FileInfo file)
        {
            Helpers.Camera.WriteJpeg(image, file);
        }

        public void SaveRaw(CameraCharacteristics characteristics, ICaptureResult result, Image image, FileInfo file)
        {
            SaveRaw(characteristics, (result as SubCCaptureResult).CaptureResult, image, file);
        }

        private void SaveRaw(CameraCharacteristics characteristics, CaptureResult result, Image image, FileInfo file)
        {
            Helpers.Camera.WriteRaw(characteristics, result, image, file);
        }
    }
}