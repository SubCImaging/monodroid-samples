//-----------------------------------------------------------------------
// <copyright file="StringToStillResolutionConverter.cs" company="SubC Imaging Ltd">
...
namespace SubCTools.Droid.Converters
{
    ...
    public class StringToStillResolutionConverter : IPropertyConverter
    {
        private StringToSizeConverter baseConverter = new StringToSizeConverter();
        public string Format => string.Join(",", StillHandler.JPEGResolutions);
        ...
        public bool TryConvert(object data, out object value)
        {
            ...
            if (!StillHandler.JPEGResolutions.Select(s => $"{s.Width}x{s.Height}").Contains($"{((Size)value).Width}x{((Size)value).Height}"))
            {
                return false;
            }
